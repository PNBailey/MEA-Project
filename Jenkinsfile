        ...
        stage('Deploy to GKE stage') {
            steps {
                script {
                    ...
                    step([$class: 'KubernetesEngineBuilder', projectId: env.PROJECT_ID, clusterName: env.CLUSTER_NAME, location: env.LOCATION, manifestPattern: 'kubernetes/deployment.yaml', credentialsId: env.CREDENTIALS_ID, namespace: "stage", verifyDeployments: true])
                }
            }
        }
        stage('QC Deploy') {
            steps {
                ...
                export STAGING_IP=\$(kubectl get svc -o json --namespace stage | jq '.items[] | select(.metadata.name == "flask-service") | .status.loadBalancer.ingress[0].ip' | tr -d '"')
